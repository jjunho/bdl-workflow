...
# Caminho absoluto para o CLI bdl.py
BDL_CLI = os.path.abspath(os.path.join(os.path.dirname(__file__), '../src/bdl.py'))

def run_cli(args, cwd=None):
    """
    Executa o CLI bdl.py com os argumentos fornecidos em um diretório específico.
    Retorna o resultado do subprocesso (stdout, stderr, returncode).
    """
    cmd = [sys.executable, BDL_CLI] + args
...
def test_init_cria_estrutura():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        nome = 'BDL'
        result = run_cli(['init', nome, '--title', 'Livro Teste', '--author', 'Autor Teste'], cwd=tmp)
